...
export const isSend = ({ tag }: ILogItem) =>
	tag === LogTag.CdpSend || tag === LogTag.DapSend;

export const isDap = ({ tag }: ILogItem) =>
	tag === LogTag.DapSend || tag === LogTag.DapReceive;
...
export const requestParams = (item: ILogItem) => {
	if (isDap(item) && item.metadata?.message?.type === "event") {
		return {
			method: item.metadata.message.event,
			params: item.metadata.message.body,
		};
	}
	...
	if (isCdp(item)) {
		return {
			method: item.metadata.message.method,
			params: item.metadata.message.params,
		};
	}

	if (isDap(item)) {
		return {
			method: item.metadata.message.command,
			params: item.metadata.message.arguments,
		};
	}
...
export const responseData = (item: ILogItem) => {
	...
	return isDap(item)
		? item.metadata.message.body
		: item.metadata.message.result;
};
